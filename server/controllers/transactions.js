...
function showAll(params, callback) {
  ...
  const before = moment.tz(params.before || new Date(), timezone).toISOString();
  const after = moment.tz(params.after || new Date(0), timezone).toISOString();
  ...
  getUniqueTransactionId(moment(transaction.date), params.userId, params.name)
    .then((uniqueId) => {
      return accounts
        ...
        .then(() => {
          return merchants.add(params.merchant, params.userId, params.name);
        })
        .then(() => {
          callback(null, {
            created: true,
...
function updateTransaction(params, callback) {
  ...
  getTransaction(params.userId, params.name, params.id)
    .then((txn) => {
      ...
      return getUniqueTransactionId(newDate, params.userId, params.name).then(
        (uniqueId) => {
...
function removeTransaction(params) {
  ...
  return getTransaction(params.userId, params.name, params.id)
    ...
    .then(() => {
      return merchants.remove(transaction.merchant, params.userId, params.name);
    });
}
